...
function journeyTitle(journey) {
  ...
  if (profile?.permit_category && ['home_purchase', 'residence_renewal', 'passport_renewal', 'uk_visa'].includes(journey?.code)) return baseTitle + ' — ' + profile.permit_category;
  return baseTitle;
}
...
function renderJourneys() {
  ...
  const activeCard = (journey) => {
    ...
    const hasDetailedTitle = Boolean(profile?.permit_category && ['home_purchase', 'residence_renewal', 'passport_renewal', 'uk_visa'].includes(journey.code));
  ...
  const completedCard = (journey) => {
    const profile = journeyProfiles[journey.id];
    const expanded = currentJourney?.id === journey.id;
    const detailedTitle = Boolean(profile?.permit_category && ['home_purchase', 'residence_renewal', 'passport_renewal', 'uk_visa'].includes(journey.code));
...
async function chooseJourney(code, startNew = false) {
  ...
  if (existing) {
    ...
    const action = await showDuplicateJourneyDialog(existing, {
      allowNew: code === 'custom_procedure' || code === 'home_purchase' || code === 'residence_renewal' || code === 'uk_visa',
      changeLabel: code === 'residence_renewal' ? 'Changer de situation' : code === 'uk_visa' ? 'Changer le type de visa' : 'Préparer un autre dossier'
    });
    if (action === 'resume') { resumeJourney(existing); return; }
    if (action === 'change' && (code === 'residence_renewal' || code === 'uk_visa')) { showQualification(code, existing, true); return; }
    ...
    if (action === 'new' && code === 'uk_visa' && typeof window.showUkVisaFlow === 'function') {
      window.showUkVisaFlow(null, true);
      return;
    }
  }
  ...
  if (code === 'uk_visa' && typeof window.showUkVisaFlow === 'function') {
    window.showUkVisaFlow();
    return;
  }
...
function showQualification(code, existingJourney = null, customResidence = false) {
  ...
  const optionalAuthorityField = definition.kind === 'residence'
    ? '<label>Département où vous habitez<select id="journey-department"><option value="">Ne pas préciser</option>' + residenceDepartments.map(([code, name]) => '<option value="' + code + '"' + ((profile ? profile.department : defaultDepartment) === code ? ' selected' : '') + '>' + code + ' — ' + escapeHtml(name) + '</option>').join('') + '</select></label>'
    : '<label>' + authorityLabel + '<input id="journey-department" placeholder="' + authorityPlaceholder + '" value="' + escapeHtml(profile ? profile.department : '') + '"></label>';
  const customDetails = '<section class="custom-simple-form"><p class="custom-step-label">VOTRE LISTE PERSONNALISÉE</p><label>Nom de votre démarche<input id="journey-custom-title" required placeholder="Ex. Acheter un terrain au pays"></label><div class="custom-requirements-field"><strong>Documents nécessaires</strong><p>Ajoutez une pièce par ligne. Jamlio vérifiera ensuite ce qui est déjà présent dans votre coffre.</p><div id="custom-requirements-list"></div><button class="outline" id="add-custom-requirement" type="button">+ Ajouter une pièce</button></div><details class="custom-optional-details"><summary>Ajouter des précisions (facultatif)</summary><div class="custom-optional-content">' + optionalAuthorityField + noteField + '</div></details></section>';
  const customDocuments = '';
  const intro = isCustom ? (isCustomResidence ? 'Indiquez votre situation et les pièces à préparer. Jamlio cherchera ensuite ce qui est déjà présent dans votre coffre.' : 'Donnez un nom à votre démarche, puis ajoutez les documents demandés. C’est tout.') : (isHome ? 'Jamlio vous aide à organiser les documents selon votre étape. Cette liste est une aide à compléter avec votre banque, votre notaire et les documents du bien.' : (definition.kind === 'residence' ? 'Choisissez la situation qui correspond le mieux à votre titre actuel. Chaque checklist standard reste reliée à sa source officielle.' : 'Choisissez d’abord le pays, puis la situation exacte de votre passeport.'));
  ...
  if (submitBar) {
    ...
    if (isCustom && node.querySelector('#custom-step-documents')) submitBar.hidden = true;
  }
  if (isCustom && node.querySelector('#custom-step-documents')) {
